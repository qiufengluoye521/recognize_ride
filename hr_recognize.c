...
#define REC_PARA_PATH "C:\\design\\C\\C_free\\recognize_ride\\rec_para.txt"
...
// 宽度权重 
static float peak_width_heighing[LEN_MAX_PEAK];
//计算最终权重
...
/**
* @brief :recognize hr data in ride bike mode
* 
* @para in: ppg_pre 			-> ppg raw data of one frame
* @para in: para 				-> para weighing
* @para in: ppg_peak_buf 		-> ppg max peak info
...
*
*/
static uint8_t ride_bike_process(int32_t 	*	ppg_pre,
							  	weighing 		para,
							  	peak_max 	* 	ppg_peak_buf,
							  	uint8_t 	* 	hr_buf,
							  	uint8_t 		flag,
...
{
	uint8_t hr_last_avg 		= 0;
	float continual_temp 		= 0;
	float max_temp 				= 0;
	uint8_t left_half_num 		= 166;
	uint8_t right_half_num 		= 0;
	uint8_t half_width 			= 0; 
	uint8_t i,jj;
	uint8_t max_heighing_num 	= 0;
	uint8_t hr_calculate;
	...
	for(i=0;i<=peak_num;i++)
	{
		...
		if(ppg_peak_buf[i].pre_num < 60)
		{
			peak_value_heighing[i] = (float)(0.2*(float)ppg_peak_buf[i].pre_value);
			
		} else if(ppg_peak_buf[i].pre_num < 80)
		{
			peak_value_heighing[i] = (float)(0.3*(float)ppg_peak_buf[i].pre_value);
		} 
		// 心率在80~180 的可能性最大
		else if(ppg_peak_buf[i].pre_num < 180)
		{
			peak_value_heighing[i] = (float)(1.0*(float)ppg_peak_buf[i].pre_value);
		}
		// 心率在180以上的可能性较小
		else
		{
			peak_value_heighing[i] = (float)(0.5*(float)ppg_peak_buf[i].pre_value);
		}
	...
	{
		// 求向左半峰值点
		jj = ppg_peak_buf[i] .pre_num;
		if(jj<45)
		{
			jj = 45;
		}
		while((float)ppg_pre[jj-45] > (float)ppg_peak_buf[i].pre_value*0.75)
		{
			jj --;
			if(jj<=44)
			{
		...
		if(right_half_num >= left_half_num)
		{
			half_width = right_half_num - left_half_num;
			peak_width_heighing[i]	= 1.0/(float)half_width;
		} else
		{
			peak_width_heighing[i] = 0;
		}
		
		if(max_temp < peak_width_heighing[i])
		{
			max_temp = peak_width_heighing[i] ;
		}		
	}
	...
	for(i=0;i<=peak_num;i++)
	{
		peak_width_heighing[i] = peak_width_heighing[i] / max_temp * 100;
	}
	...
	for(i=0;i<=peak_num;i++)
	{
		total_heighing[i] = continual_heighing[i]*para.continuity  \
						+ peak_value_heighing[i]*para.peak_amplitude \
						+ peak_width_heighing[i]*para.peak_width;
		if(max_temp < total_heighing[i])
		{
			max_temp 			= total_heighing[i];
			max_heighing_num	= i;
...
void hr_ride_recognize_fun(int32_t * ppg_fft_out)
{
	uint8_t i = 0,j = 0,jj = 0;
	uint8_t k = 0;
	...
	{
		// 决断极值条件
		if((ppg_fft_out[j] >= ppg_fft_out[j-1]) && (ppg_fft_out[j-1] >= ppg_fft_out[j-2]) \
			&& (ppg_fft_out[j] >= ppg_fft_out[j+1]) && (ppg_fft_out[j+1] >= ppg_fft_out[j+2]))
		{
			for(jj = 0;jj < LEN_MAX_PEAK;jj++)
			{
				if(ppg_fft_out[j] > ppg_max_peak_buff[jj].pre_value)
	...
	/*start recognize hr value in ride bike mode*/
	weighing_parameter.continuity 			= 25;
	weighing_parameter.peak_amplitude		= 30;
	weighing_parameter.peak_width			= 35;
	weighing_parameter.move_factor			= 0;
	weighing_parameter.rise_factor			= 0;
	weighing_parameter.hr_value_factor 		= 35;
	...
	hr = ride_bike_process(ppg_fft_out,weighing_parameter,ppg_max_peak_buff,hr_last_3_buf,is_first_3_hr_flag,LEN_MAX_PEAK-1);
	printf("hr_%d :%d\r\n",fft_count,hr);
	hr_last_3_buf[0] = hr_last_3_buf[1];
	hr_last_3_buf[1] = hr_last_3_buf[2];
	hr_last_3_buf[2] = hr;
